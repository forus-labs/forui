...
import '../transformation/argument_elision.dart';
import 'routes.dart';
import 'widgets.dart';
...
class Examples extends RecursiveAstVisitor<void> {
  static Future<Map<String, Snippet>> transform(
    ...
    final main = p.join(docsSnippets, 'main.dart');
    ...
    for (final path in paths) {
      ...
      for (final file in dir.listSync(recursive: true).whereType<File>().where((f) => f.path.endsWith('.dart'))) {
        if (await session.getResolvedUnit(file.path) case final ResolvedUnitResult result) {
          final visitor = Examples._(session, overlay, result, snippets, packages);
  ...
  Examples._(this._session, this._overlay, this._result, this._snippets, this._packages);
