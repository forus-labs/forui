...
Future<void> main() async {
  ...
  final main = p.join(_samples, 'main.dart');
  ...
  for (final file in Directory(_samples).listSync(recursive: true).whereType<File>()) {
...
class SampleVisitor extends RecursiveAstVisitor<Future<void>> {
  ...
  Future<void> visitClassDeclaration(ClassDeclaration node) async {
    ...
    if (inline == null) {
      (_samples[node.name.lexeme] ??= Sample()).source = await _include(
        _result,
        inclusions,
        node,
