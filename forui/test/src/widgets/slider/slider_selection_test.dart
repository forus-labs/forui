  ...
  group('ContinuousSelection', () {
    ...
    group('step', () {
      test('min step negative', () {
        expect(
          value(0.1, 0.2, step: 0.5).step(min: true, extend: true),
          value(0.0, 0.2, step: 0.5),
        );
      });

      test('max step > extent', () {
        expect(
          value(0.7, 0.8, step: 0.5).step(min: false, extend: true),
          value(0.7, 1.0, step: 0.5),
        );
      });

      test('min step > max raw offset', () {
        expect(
          value(0.2, 0.4, step: 0.5).step(min: true, extend: false),
          value(0.3, 0.4, step: 0.5),
        );
      });

      test('max step < max min offset', () {
        expect(
          value(0.2, 0.4, step: 0.5).step(min: false, extend: false),
          value(0.2, 0.3, step: 0.5),
        );
      });
      ...
      test(
        'min extend',
        () => expect(
          value(0.2, 0.4).step(min: true, extend: true),
          value(0.15, 0.4),
        ),
      );
      ...
      test(
        'min shrink',
        () => expect(
          value(0.2, 0.4).step(min: true, extend: false),
          value(0.25, 0.4),
        ),
      );
      ...
      test(
        'max extend',
        () => expect(
          value(0.2, 0.4).step(min: false, extend: true),
          value(0.2, 0.45),
        ),
      );
      ...
      test(
        'max shrink',
        () => expect(
          value(0.2, 0.4).step(min: false, extend: false),
          value(0.2, 0.35),
        ),
      );
    });
    ...
    group('move', () {
      test(
        'min negative offset',
        () => expect(value(0.2, 0.4).move(min: true, to: -1), value(0.1, 0.4)),
      );

      test(
        'max negative offset',
        () => expect(value(0.2, 0.4).move(min: false, to: -1), value(0.2, 0.3)),
      );

      test(
        'min offset > extent',
        () => expect(value(0.2, 0.4).move(min: true, to: 101), value(0.3, 0.4)),
      );

      test(
        'max offset > extent',
        () =>
            expect(value(0.2, 0.4).move(min: false, to: 101), value(0.2, 0.5)),
      );

      test(
        'min offset < min raw extent',
        () => expect(value(0.2, 0.4).move(min: true, to: 35), value(0.3, 0.4)),
      );

      test(
        'min offset > max raw extent',
        () => expect(value(0.2, 0.4).move(min: true, to: 0), value(0.1, 0.4)),
      );

      test(
        'min offset',
        () => expect(value(0.2, 0.4).move(min: true, to: 25), value(0.25, 0.4)),
      );

      test(
        'max offset < min raw extent',
        () => expect(value(0.2, 0.4).move(min: false, to: 29), value(0.2, 0.3)),
      );

      test(
        'max offset > max raw extent',
        () => expect(value(0.2, 0.4).move(min: false, to: 51), value(0.2, 0.5)),
      );

      test(
        'max offset < min raw extent',
        () =>
            expect(value(0.2, 0.4).move(min: false, to: 45), value(0.2, 0.45)),
      );
  ...
  group('DiscreteSelection', () {
    ...
    group('step', () {
      ...
      for (final (min, extend) in [
        ...
        test(
          'single point - ${min ? 'min' : 'max'} ${extend ? 'extend' : 'shrink'}',
          () {
            expect(
              value(
                0.5,
    ...
    group('move', () {
      test(
        'min on tick',
        () =>
            expect(value(0.25, 0.5).move(min: true, to: 25), value(0.25, 0.5)),
      );

      test(
        'max on tick',
        () => expect(value(0.0, 0.5).move(min: false, to: 50), value(0.0, 0.5)),
      );

      test(
        'min shrink beyond extent',
        () => expect(
          value(0.5, 0.875).move(min: true, to: 87.5),
          value(0.75, 0.875),
        ),
      );

      test(
        'max shrink beyond extent',
        () => expect(
          value(0.125, 0.5).move(min: false, to: 12.5),
          value(0.125, 0.25),
        ),
      );
      ...
      test('min shrink, no min extent', () {
        expect(
          value(
            0.25,
            0.5,
            extent: (min: 0, max: 0.45),
          ).move(min: true, to: 100),
      ...
      for (final (index, (value, to, expected))
        ...
        test(
          '[$index] min extend/shrink',
          () => expect(value.move(min: true, to: to), expected),
        );
      }
      ...
      for (final (index, (value, to, expected))
        ...
        test(
          '[$index] max extend/shrink',
          () => expect(value.move(min: false, to: to), expected),
        );
      }
    });
  });
  ...
  group('SplayTreeMaps', () {
    ...
    group('round(...)', () {
      ...
      test(
        'up - only one element',
        () => expect(
          SplayTreeMap<double, void>.from({1.0: null}).round(0.3),
          1.0,
        ),
      );
      ...
      test(
        'down - only one element',
        () => expect(
          SplayTreeMap<double, void>.from({1.0: null}).round(1.3),
          1.0,
        ),
      );
      ...
      test(
        'empty',
        () => expect(
          () => SplayTreeMap<double, void>().round(1.3),
          throwsArgumentError,
        ),
      );
    });
