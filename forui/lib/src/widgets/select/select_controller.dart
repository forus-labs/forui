...
class SelectControllerData<T> extends InheritedWidget {
  ...
  static SelectControllerData<T> of<T>(BuildContext context) {
    final SelectControllerData<T>? result =
        context.dependOnInheritedWidgetOfExactType<SelectControllerData<T>>();
    assert(
      result != null,
  ...
  bool updateShouldNotify(SelectControllerData<T> old) =>
      contains != old.contains || onPress != old.onPress;
...
class FSelectController<T> extends FValueNotifier<T?> {
  ...
  @override
  set value(T? value) =>
      super.value = toggleable && super.value == value ? null : value;
