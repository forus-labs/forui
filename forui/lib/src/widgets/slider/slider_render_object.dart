...
class HorizontalSliderRenderObject extends _SliderRenderObject {
  ...
  RenderObject createRenderObject(BuildContext context) {
    ...
    return _RenderHorizontalSlider(
      style,
      layout,
      direction,
      labelledMarks,
...
abstract class _SliderRenderObject extends MultiChildRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    covariant _RenderSlider slider,
  ) {
    final InheritedData(
      :style,
      :layout,
  ...
  void performLayout() {
    ...
    final (:label, :paddedTrack, :description, :error, :slider) = layoutParts(
      ...
      switch (_mainAxisExtent) {
        final extent? => loosened.copyWith(
          maxWidth: extent + insets.left + insets.right,
        ),
        null => loosened,
      },
    );
    ...
    final width =
        [
          ...
          description.size,
          error.size,
        ].order(by: (size) => size.width).max!.width;
...
class _RenderVerticalSlider extends _RenderSlider {
  ...
  void performLayout() {
    ...
    final (:label, :paddedTrack, :description, :error, :slider) = layoutParts(
      ...
      switch (_mainAxisExtent) {
        final extent? => loosened.copyWith(
          maxHeight: extent + insets.top + insets.bottom,
        ),
        null => loosened,
      },
    );
    ...
    if (largestMiddleOffset < marksOffset) {
      ...
      sliderOffset = Offset(
        largestMiddleOffset - paddedTrack.size.width / 2,
        label.size.height,
      );
      largestOffset = 0.0;
    }
    ...
    largest.data.offset = Offset(
      max(largestOffset - largest.size.width / 2, 0),
      0,
    );
...
abstract class _RenderSlider extends RenderBox
  ...
  late Rect Function(RenderBox, Size, FSliderMark, FSliderMarkStyle)
  _positionMark;
  ...
  _Parts layoutParts(BoxConstraints constraints, BoxConstraints trackMainAxis) {
    final paddedTrack =
        firstChild!..layout(trackMainAxis, parentUsesSize: true);

    final label =
        childAfter(paddedTrack)!..layout(constraints, parentUsesSize: true);
    final description =
        childAfter(label)!..layout(constraints, parentUsesSize: true);
    final error =
        childAfter(description)!..layout(constraints, parentUsesSize: true);

    final (marks, sliderSize) = _layoutMarks(
      constraints,
      paddedTrack,
      childAfter(error),
    );
  ...
  (Map<RenderBox, Rect>, Size) _layoutMarks(
    ...
    for (final mark in _marks) {
      ...
      final rect = positionMark(
        paddedTrack,
        label.size,
        mark,
        mark.style ?? _style.markStyle,
      );
      marks[label] = rect;
  ...
  (double, double) _anchor(
    ...
    final crossAxisExtent =
        _style.crossAxisExtent < thumb ? thumb : _style.crossAxisExtent;
    final crossAxisOffset =
        crossAxisPadding + (markStyle.labelOffset < 0 ? 0.0 : crossAxisExtent);
  ...
  Rect _rect(Size size, Offset anchor, FSliderMarkStyle markStyle) {
    ...
    return rect.shift(
      anchor -
          markStyle.labelAnchor
              .resolve(_textDirection)
              .relative(to: size, origin: anchor),
    );
  }
