...
class _Calendar extends MultiChildRenderObjectWidget {
  _Calendar({
    required List<Widget> children, Key? key,
  }) : super(key: key, children: children);

  @override
  RenderObject createRenderObject(BuildContext context) => _Speculative();
}
...
class _Speculative extends RenderBox
    with ContainerRenderObjectMixin<RenderBox, _Data>, RenderBoxContainerDefaultsMixin<RenderBox, _Data> {
  @override
  void performLayout() {
    final unselected = firstChild!;
    final selected = childAfter(unselected)!;
    final viewport = childAfter(selected)!;

    final a = unselected.getDryLayout(constraints);
    final b = selected.getDryLayout(constraints);

    final constrained = constraints.copyWith(maxHeight: max(a.height, b.height) + 3);

    viewport.layout(constrained, parentUsesSize: true);
