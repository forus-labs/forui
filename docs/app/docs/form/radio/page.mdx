...
<div className="pb-5">
  <Callout type="info">
    We recommend using a [select group](/docs/form/select-group#radio-form) to create a group of radio buttons instead
    of using `FRadio` directly.
  </Callout>
</div>
...
<Tabs items={['Preview', 'Code']}>
  ...
  <Tabs.Tab>
    ```dart copy FRadio( label: const Text('Default'), description: const Text('The description of the default
    option.'), value: true, onChange: (value) {}, ); ```
  </Tabs.Tab>
</Tabs>
