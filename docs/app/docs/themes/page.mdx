...
To generate a `FButton`'s styles:
...
This will generate:
```dart filename="lib/theme/button_styles.dart" {25-28} copy
...
FButtonStyle _buttonStyle({
  ...
  decoration: FWidgetStateMap({
    // We want to add a decoration for a disabled button when it is hovered or pressed.
    ...
    // Multiple states can be combined to form more complex constraints.
    // This constraint is equivalent to: `WidgetState.hovered` or `WidgetState.pressed`.
    // See https://api.flutter.dev/flutter/widgets/WidgetStatesConstraint-class.html
...
This will generate a theme which you can add the generated button styles to:
...
FThemeData get zincLight {
  ...
  return FThemeData(
    ...
    buttonStyles: buttonStyles(colors: colors, style: style),
  );
}
