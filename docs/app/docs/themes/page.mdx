...
FAccordionStyle --> FWidgetStateMap
```
...
- **[`FWidgetStateMap`](https://pub.dev/documentation/forui/latest/forui.theme/FWidgetStateMap-class.html)**: Allows you to define different values for different widget states (e.g. hovered, pressed).
...
<Callout type="info">
  **Want to start using themes right away?** Jump to [Getting Started](#getting-started) to use predefined themes or
  generate your own. The sections ahead explain the core components in detail.
</Callout>
...
Run to generate a theme based on [zinc](#predefined-themes)'s light variant:
```shell copy
