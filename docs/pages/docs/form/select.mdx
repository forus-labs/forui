...
FSelect<String>(
  ...
  prefixBuilder: (context, styles, _) => FIcon(FAssets.icons.globe),
  suffixBuilder: (context, styles, _) => FIcon(FAssets.icons.arrowDown),
  ...
  children: [
    FSelectSection(
      title: const Text('North American Countries'),
      children: [
        FSelectItem(
          value: 'us',
...
<Tabs items={['Preview', 'Code']}>
  <Tabs.Tab>
    <Widget name='select' variant='no-scroll-handles' query={{}} height={400}/>
  </Tabs.Tab>
  <Tabs.Tab>
    ```dart {5} copy
    class NoScrollHandlesSelectPage extends StatelessWidget {
      @override
      Widget build(BuildContext context) => FSelect<String>(
        hint: 'Select a fruit',
        contentScrollHandles: false, 
