...
<Callout type="warning">
  The `toMaterialTheme()` method is marked as experimental. The mapping between Forui and Material themes is done on a best-effort basis, and may not perfectly capture all the nuances of a Forui theme.
</Callout>

### Usage

A Forui theme can be converted to a Material theme using the `toMaterialTheme()` method.
...
Widget build(BuildContext context) {
  ...
  return MaterialApp(
    theme: fThemeData.toMaterialTheme(),
    home: Scaffold(
      body: Center(
        child: FCard(
          title: 'Mixed Widgets',
